...
$orderItemsQuery = mysqli_query($conn, "
    SELECT od.Quantity, od.Price, od.SpecialNotes ,p.FoodName, p.ProductID
    FROM foodorder_details od
    JOIN foodproduct p ON od.ProductID = p.ProductID
...
<div class = "users-details-container">
    ...
    <div class="section">
        ...
        <?php
        $orderItemsQuery = mysqli_query($conn, "
            SELECT od.Quantity, od.Price, p.FoodName, p.ProductID
            FROM foodorder_details od
            JOIN foodproduct p ON od.ProductID = p.ProductID
            WHERE od.OrderID = '$orderID'
        ");
        ...
        while ($item = mysqli_fetch_assoc($orderItemsQuery)):
            ...
            $imgResult = mysqli_query($conn, "SELECT ImagePath FROM foodproduct WHERE ProductID = '$productID' LIMIT 1");
        ...
        ?>
            <div class="order-item" style="display: flex; align-items: center;">
                ...
                <div style="flex: 1;">
                    <strong><?= htmlspecialchars($item['FoodName']) ?></strong><br>
                    Quantity: <?= $item['Quantity'] ?> | Price: R<?= number_format($item['Price'], 2) ?>
                    <br><a href="treasure_hub_dev_foodproductdisplay.php?productid=<?= $productID ?>">View Product</a>
                </div>
            </div>
        <?php endwhile; ?>
    </div>
