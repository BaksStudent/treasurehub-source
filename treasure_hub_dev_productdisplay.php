...
if (!empty($search)) 
{
    ...
    $imageQuery = mysqli_query($conn, "SELECT * FROM productimages WHERE ProductID = $productID LIMIT 4");
    $images = [];
...
<?php
    ...
    if (isset($_POST['delist-btn']))
    {
        ...
        $update= "UPDATE product SET isSold = 1 WHERE ProductID = $productID";
        $date = date("Y-m-d");
        $description = "Delisted product number ".$productID;
...
<div class="container">
    <div class="section">
        ...
        <p><strong>Short Description:</strong> <?= htmlspecialchars($product['shortDescription']) ?></p>
        <p><strong>Long Description:</strong> <?= htmlspecialchars($product['longDescription']) ?></p>
    </div>
    ...
    <div class="section">
        ...
        <div class="product-images">
            <?php foreach ($images as $img): ?>
                <img src="Images/<?= htmlspecialchars($img['ImagePath']) ?>" alt="<?= htmlspecialchars($img['altText']) ?>">
            <?php endforeach; ?>
        </div>
    </div>
